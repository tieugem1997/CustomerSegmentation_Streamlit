...
if choice == 'Business Understanding':
    ...
    if st.session_state['df'] is not None:
        ...
        # Display the 5 most recent feedbacks
        if os.path.isfile('feedback_model.csv'):
            all_feedbacks = pd.read_csv('feedback_model.csv')
            all_feedbacks.sort_values('Time', ascending=False, inplace=True)
            st.write("### 5 Most Recent Feedbacks:")
...
elif choice == 'BigData: Spark':
    st.title('BigData: Spark')
    from pyspark.ml.clustering import KMeans
    # Tải và cấu hình Apache Spark (bạn cần cấu hình đúng đường dẫn cho biến môi trường)
    os.environ["JAVA_HOME"] = "C:/Program Files/Java/jdk-1.8"
    os.environ["SPARK_HOME"] = "C:/Spark/spark-3.4.1-bin-hadoop3"

    # Tích hợp findspark và khởi tạo SparkSession
    findspark.init()

    SparkContext.setSystemProperty('spark.hadoop.dfs.client.use.datanode.hostname', 'true')
    CustomerSegmentation = SparkConf().setAppName("Project 1 Customer Segmentation").setMaster("local[*]")
    sc = SparkContext.getOrCreate(conf= CustomerSegmentation)

    spark = SparkSession.builder \
        .appName("Spark RFM Analysis") \
        .getOrCreate()
    ...
    if st.session_state['df'] is not None:
        ...
        st.write("### Elbow Plot for Optimal Number of Clusters")
        ...
        optimal_k = st.selectbox('Select the optimal number of clusters:', k_list)
        
        # Modeling KMeans with the optimal number of clusters chosen by the user
        kmeans_spark = KMeans(featuresCol='features', k=optimal_k)
        model_spark = kmeans_spark.fit(final_data)

        predictions = model_spark.transform(final_data)
        evaluator = ClusteringEvaluator()
        silhoutte = evaluator.evaluate(predictions)
        st.write(f'Silhouette Score: {silhoutte}')
        ...
        squarify.plot(sizes=rfm_agg_big['Count'],
                        ...
                        label=[f'{rfm_agg_big.iloc[i]["prediction"]} \n{rfm_agg_big.iloc[i]["RecencyMean"]} days \n{rfm_agg_big.iloc[i]["FrequencyMean"]} orders \n{rfm_agg_big.iloc[i]["MonetaryMean"]} $ \n{rfm_agg_big.iloc[i]["Count"]} customers ({rfm_agg_big.iloc[i]["Percent"]}%)'
                                for i in range(len(rfm_agg_big))], alpha=0.5)
